...
export const getTreatmentLists = async (req, res) => {
  try {
    ...
    console.error(err);
    return res.status(500).json({ staus: false, message: 'Server error', error: err.message,data:nul });
  }
};
...
export const getIndividualTreatment = async (req, res) => {
  try {
    ...
    if (!treatmentList) {
      return res.status(404).json({ status:false, message: 'Treatment list not found',data:null });
    }
    ...
    if (treatmentId) {
      if (Number.isInteger(parseInt(treatmentId))) {
        ...
        if (!treatment) {
          return res.status(404).json({status:false, message: 'Treatment not found at this index' , data:null });
        }
      }
      else {
        treatment = treatmentList.treatments.find(t => t.serviceName === treatmentId);
        if (!treatment) {
          return res.status(404).json({status:false, message: 'Treatment with this serviceName not found',data:null });
        }
      }
    } else {
      return res.status(400).json({ message: 'Treatment identifier is required' });
    }
